...
var overrideExtensionsRegisterAction = function overrideExtensionsRegisterAction() {
  ...
  if (!api._originalRegisterAction) {
    ...
    app.dv.cache.extensions.registerAction = function (action) {
      ...
      api._originalRegisterAction(action); // Replace each code block with its transformed version and evaluate it so the transformed code executes when called
...
if (api._supportLegacyBrowser !== true && api.utils.supportsES6()) {
  ...
  api._supportLegacyBrowser = true; // Load Babel to transform JavaScript for older browsers
