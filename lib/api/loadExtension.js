...
var _apiUtils2 = _interopRequireDefault(require("./internal/_apiUtils"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }

function loadExtension() {
  var context = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : window || {};
  (0, _apiUtils2["default"])(context); // NOTE: All code herein must be run before other extension modules as they depend on it.
  ...
  var overrideExtensionsRegisterAction = function overrideExtensionsRegisterAction() {
    ...
    if (!context.api._originalRegisterAction) {
      context.api._originalRegisterAction = app.dv.cache.extensions.registerAction;

      app.dv.cache.extensions.registerAction = function (action) {
        // Call the original function to register the actions
        context.api._originalRegisterAction(action); // Replace each code block with its transformed version and evaluate it so the transformed code executes when called
  ...
  var overrideRunJavaScriptActionExecute = function overrideRunJavaScriptActionExecute() {
    ...
    if (!context.api._originalRunJavaScriptActionExecute) {
      context.api._originalRunJavaScriptActionExecute = app.dv.actionLibrary.actionStores.dvCore.RunJavaScript.Definition.Execute;
  ...
  if (context.api._supportLegacyBrowser !== true && context.api.utils.supportsES6()) {
    ...
    context.api._supportLegacyBrowser = true; // Load Babel to transform JavaScript for older browsers
...
//# sourceMappingURL=loadExtension.js.map