...
var _default = function () {
   ...
   */
  api.loadExtension("api.actions", function () {
    ...
    if (!api.actions.runTaskAsync) {
       ...
       */
      api.actions.runTaskAsync = function (formID, taskID) {
        var mode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : api.actions.FormRequestBehaviorModes.Normal;
        return new Promise(function (resolve, reject) {
          // Function to run the task after we have the form
          var runTask = function runTask(form) {
            app.dv.actionLibrary.inlineExe(form, app.dv.actionLibrary.actionStores.dvCore.CallActionChain, {
              viewID: formID,
              macroID: taskID
            }, function (result) {
              if (result.success) {
                ...
                reject("The task (ID: ".concat(taskID, ") on form (ID: ").concat(formID, ") could not be executed.").concat(result.actionMessage ? " Action Message: " + result.actionMessage : ""));
              }
            });
          }; // Assume the form is already open, and find it


          var form = api.utils.getOpenForm(formID);
          ...
          if (form) {
            ...
            if (mode !== api.actions.FormRequestBehaviorModes.Fail) {
              api.utils.openFormAsync(formID, mode).then(function (f) {
                runTask(f);
              });
            } else {
              reject("The form (ID: ".concat(formID, ") was not already open, and it could not be opened, so the task could not be run."));
            }
          }
        });
      };
    ...
    if (!api.actions.runTask) {
       ...
       * @param {number} [mode=api.actions.FormRequestBehavior.Normal]     One of the [api.actions.FormRequestBehaviorModes] to use when opening the form.
       */
      api.actions.runTask = function (formID, taskID) {
        var mode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : api.actions.FormRequestBehaviorModes.Normal;
