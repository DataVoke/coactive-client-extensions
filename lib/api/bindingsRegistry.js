...
api.loadExtension("api.bindingsRegistry", function () {
  ...
  if (!api.bindingsRegistry.register) {
     ...
     */
    api.bindingsRegistry.register = function (_ref) {
      var key = _ref.key,
          onRegister = _ref.onRegister,
          onDeregister = _ref.onDeregister,
          onExecute = _ref.onExecute,
          executionInterval = _ref.executionInterval,
          deregisterOnExecute = _ref.deregisterOnExecute,
          registrationFormID = _ref.registrationFormID;
      var bindings = api.bindingsRegistry.bindings; // Look for the item in the registry

      var registryItem = bindings.get(key);
      ...
      registryItem = {
        ...
        onExecute: onExecute,
        deregisterOnExecute: deregisterOnExecute,
      ...
      bindings.set(key, registryItem); // Start time-based execution if interval present

      executionInterval = Number.parseInt(executionInterval);

      if (executionInterval) {
        registryItem.executionIntervalHandle = window.setInterval(function () {
          return api.bindingsRegistry.execute({
            key: key
          });
        }, executionInterval);
      }
  ...
  if (!api.bindingsRegistry.deregister) {
     ...
     */
    api.bindingsRegistry.deregister = function (key) {
      var bindings = api.bindingsRegistry.bindings;
      var deregistrationResult = null; // Look for the item in the registry

      var registryItem = bindings.get(key);

      if (registryItem) {
        // Run any deregistration and remove it from the registry
        deregistrationResult = api.utils.runFunction(registryItem.onDeregister, null, [registryItem]); // Stop interval-based execution if necessary
        ...
        bindings["delete"](key); //console.log(`binding deleted '${key}'`);
      } else {//console.log(`No binding has been registered with the key '${key}'.`);
        }
  ...
  if (!api.bindingsRegistry.execute) {
     ...
     */
    api.bindingsRegistry.execute = function (_ref2) {
      var key = _ref2.key,
          executionFormID = _ref2.executionFormID,
          deregister = _ref2.deregister,
          passedValue = _ref2.passedValue;
      var bindings = api.bindingsRegistry.bindings;
      ...
      if (registryItem) {
        // Store the executor
        registryItem.executionFormID = executionFormID; // Make passed value available to listeners

        registryItem.passedValue = passedValue; // Run execution function

        executionResult = api.utils.runFunction(registryItem.onExecute, null, [registryItem]); // Save execution result in case listeners need it

        registryItem.executionResult = executionResult; // Deregister if binding item flag or override parameter indicates to do so
  ...
  if (!api.bindingsRegistry.getExecutionResult) {
    api.bindingsRegistry.getExecutionResult = function (key) {
      var executionResult = null;
      var bindings = api.bindingsRegistry.bindings; // Look for the item in the registry
  ...
  if (!api.bindingsRegistry.getPassedValue) {
    api.bindingsRegistry.getPassedValue = function (key) {
      var passedValue = null;
      var deregistrationResult = null;
      var bindings = api.bindingsRegistry.bindings; // Look for the item in the registry
      ...
      if (registryItem) {
        ...
        registryItem.executedPassedValue = true; // If the flag was set to delay deregistration until after passedValue has been read,
...
//# sourceMappingURL=bindingsRegistry.js.map